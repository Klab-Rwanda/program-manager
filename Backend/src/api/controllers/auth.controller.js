...
const bulkRegisterUsers = asyncHandler(async (req, res) => {
  ...
  if (!req.file) {
    throw new ApiError(
      400,
      'Excel/CSV file is required for bulk registration.'
    );
  }
  ...
  if (req.user.role === 'Program Manager' && targetRole !== 'Trainee') {
    throw new ApiError(
      403,
      'Forbidden: Program Managers can only bulk register Trainees.'
    );
  }
  ...
  if (req.user.role !== 'SuperAdmin' && req.user.role !== 'Program Manager') {
    throw new ApiError(
      403,
      'Forbidden: Only Program Managers or Super Admins can bulk register users.'
    );
  }
  ...
  try {
    ...
    throw new ApiError(
      400,
      "Could not read file. Ensure it's a valid Excel/CSV format."
    );
  } finally {
  ...
  if (jsonData.length < 2) {
    ...
    throw new ApiError(
      400,
      'File is empty or contains only headers. Please provide data.'
    );
  }
  ...
  const registrationPromises = usersToRegister.map(async (row, index) => {
    ...
    try {
      ...
      results.failed++;
      console.error(
        `Error processing row ${index + 2} (${email}):`,
        innerError
      );
      results.errors.push({
        row: index + 2,
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    throw new ApiError(404, 'User does not exist or not activated.');
  }
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    throw new ApiError(
      404,
      'User with this email does not exist or not activated'
    );
  }
